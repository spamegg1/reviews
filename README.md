...
Part 5 is about React frontend testing and End-to-End testing. We add features to the frontend to interact with the backend functionality from Parts 3 and 4 (using authentication, posting new notes etc.). Then we learn about child components and lifting up state that is shared between parent and child components, also passing state down to child components whenever possible, to simplify the main `App.js` file. To unit-test the frontend rendering,  `react-testing-library` is used. For end-to-end testing [Cypress](https://www.cypress.io/) is used (instead of something like the very popular but complicated [Selenium](https://www.selenium.dev/)).

[Testing JavaScript](https://testingjavascript.com/) describes 4 fundamental steps to flawless testing: static (type checking), unit, integration, and end-to-end. In Part 5 we skip the first: we do not learn about TypeScript yet; and the third: the lectures mention that we **should** do integration testing of multiple components working together, but that it would be way too difficult and complicated and it's beyond the scope of the course.
