...
class JackTokenizer():
    ...
    def __init__(self, filename):
        ...
        self.tokens = []  # list of tokens, initially empty
            ...
            if line_without_comments[i] in ['/*', '/**']:
                ...
                if line_without_comments:
                    while line_without_comments[i] != '*/':
                        line_without_comments.pop(i)
                        if i == len(line_without_comments):
                            break
                    if i < len(line_without_comments):
...
def split_symbols(word):
    ...
    # if word is a string token (starts and ends with double quotes), ignore
    if word[0] == '"':
        return [word]
...
# run main
# main()
# print(handle_strings(['do', 'Output.printString("null");']))
# print(handle_strings('"Test: expected result: 5; actual result: "'.split()))
