...
/** A huffman code is represented by a binary tree.
  *
  * Every `Leaf` node of the tree represents one character of the alphabet that the tree
  * can encode. The weight of a `Leaf` is the frequency of appearance of the character.
  *
  * The branches of the huffman tree, the `Fork` nodes, represent a set containing all the
  * characters present in the leaves below it. The weight of a `Fork` node is the sum of
  * the weights of these leaves.
  */
...
trait Huffman extends HuffmanInterface:
  ...
  /** This function computes for each unique character in the list `chars` the number of
    ...
    *
    * In order to access the two elements of a pair, you can use the accessors `_1` and
    * `_2`:
    *
    ...
    */
  def times(chars: List[Char]): List[(Char, Int)] = chars // TODO
    .groupMapReduce(identity)(_ => 1)(_ + _)
    .toList

  /** Returns a list of `Leaf` nodes for a given frequency table `freqs`. The returned
    * list should be ordered by ascending weights (i.e. the head of the list should have
    * the smallest weight), where the weight of a leaf is the frequency of the character.
    */
  def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] = freqs // TODO
    .view
    .sortBy(_._2)
    .map(Leaf(_, _))
    .toList
  ...
  /** The parameter `trees` of this function is a list of code trees ordered by ascending
    * weights. This function takes the first two elements of the list `trees` and combines
    * them into a single `Fork` node. This node is then added back into the remaining
    * elements of `trees` at a position such that the ordering by weights is preserved. If
    * `trees` is a list of less than two elements, that list should be returned unchanged.
    */
  ...
  /** This function will be called in the following way: until(singleton, combine)(trees)
    * where `trees` is of type `List[CodeTree]`, `singleton` and `combine` refer to the
    * two functions defined above. In such an invocation, `until` should call the two
    * functions until the list of code trees contains only one single tree, and then
    * return that singleton list.
    */
  @annotation.tailrec
  final def until(
  ...
  /** This function creates a code tree which is optimal to encode the text `chars`. The
    * parameter `chars` is an arbitrary text. This function extracts the character
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = // TODO
    ...
    def helper(t: CodeTree, b: List[Bit], acc: List[Char]): List[Char] = t match
      ...
      case Fork(left, right, _, _) =>
        b.head match // b should not be empty here
  ...
  /** What does the secret message say? Can you decode it? For the decoding use the
    * `frenchCode' Huffman tree defined above.
    */
      ...
      case Fork(left, right, _, _) =>
        (left, right) match
          case (Leaf(c, _), Leaf(_, _)) =>
            if c == char then 0 :: lookup(left)(char) else 1 :: lookup(right)(char)

          case (Fork(l, r, chars, _), Leaf(c, _)) =>
            if chars.contains(char) then 0 :: lookup(left)(char)
            else 1 :: lookup(right)(char)

          case (Leaf(c, _), Fork(l, r, chars, _)) =>
            if chars.contains(char) then 1 :: lookup(right)(char)
  ...
  /** This function returns the bit sequence that represents the character `char` in the
    * code table `table`.
    */
  def codeBits(table: CodeTable)(char: Char): List[Bit] = table // TODO
    .find(_._1 == char)
    .get
    ._2

  /** Given a code tree, create a code table which contains, for every character in the
    * code tree, the sequence of bits representing that character. Hint: think of a
    * recursive solution: every sub-tree of the code tree `tree` is itself a valid code
    * tree that can be represented as a code table. Using the code tables of the
  ...
  /** This function takes two code tables and merges them into one. Depending on how you
    * use it in the `convert` method above, this merge method might also do some
  ...
  /** This function encodes `text` according to the code tree `tree`. To speed up the
    * encoding process, it first converts the code tree to a code table and then uses it
