...
object TweetReader:
  object ParseTweets:
    def regexParser(s: String): List[Map[String, Any]] =
      ...
      val tweetRegex: Regex = // recursive val???
        """^\{ .*"user": "([^"]+)", "text": "([^"]+)", "retweets": ([\\.0-9]+) \},?$""".r
      ...
      s
        ...
        .tail
        .init
        .map { case tweetRegex(user, text, retweets) =>
          Map("user" -> user, "text" -> text, "retweets" -> retweets.toDouble)
        }

    def getTweets(user: String, json: String): List[Tweet] =
      for map <- regexParser(json)
      yield
        val text = map("text")
    ...
    def getTweetData(user: String, json: String): List[Tweet] =
      val l = regexParser(json) // is list
  ...
  def toTweetSet(l: List[Tweet]): TweetSet = l.foldLeft(Empty(): TweetSet)(_.incl(_))
