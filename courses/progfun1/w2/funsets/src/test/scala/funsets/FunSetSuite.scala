...
/** This class is a test suite for the methods in object FunSets. To run this test suite,
...
class FunSetSuite extends munit.FunSuite:
  ...
  /** When writing tests, one would often like to re-use certain values for multiple
    * tests. For instance, we would like to create an Int-set and have multiple test about
    * it.
    *
    * Instead of copy-pasting the code for creating the set into every test, we can store
    * it in the test class using a val:
    *
    * val s1 = singletonSet(1)
    *
    * However, what happens if the method "singletonSet" has a bug and crashes? Then the
    * test methods are not even executed, because creating an instance of the test class
    * fails!
    *
    * Therefore, we put the shared values into a separate trait (traits are like abstract
    * classes), and create an instance inside each test method.
    */
  ...
  /** This test is currently disabled (by using @Ignore) because the method "singletonSet"
    * is not yet implemented and the test would fail.
    *
  ...
  test("singleton set one contains one"): // TODO
    ...
    /** We create a new instance of the "TestSets" trait, this gives us access to the
      * values "s1" to "s3".
      */
    new TestSets:
      /** The string argument of "assert" is a message that is printed in case the test
        * fails. This helps identifying which assertion failed.
        */
  ...
  test("map: squares of negatives are all positive"): // TODO
    new TestSets:
      val negsSquared = map(negSet, sq)
      assertEquals(forall(negsSquared, isPos), true, "Map negs sq")

  test("map: adding 1 to odds make them all even"): // TODO
    new TestSets:
