...
trait FunSets extends FunSetsInterface:
  ...
  /** Returns the union of the two given sets, the sets of all elements that are in either
    * `s` or `t`.
    */
  def union(s: FunSet, t: FunSet): FunSet = n => s(n) || t(n) // TODO

  /** Returns the intersection of the two given sets, the set of all elements that are
    * both in `s` and `t`.
    */
  def intersect(s: FunSet, t: FunSet): FunSet = n => s(n) && t(n) // TODO

  /** Returns the difference of the two given sets, the set of all elements of `s` that
    * are not in `t`.
    */
