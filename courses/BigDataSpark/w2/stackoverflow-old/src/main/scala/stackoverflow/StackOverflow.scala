...
class StackOverflow extends StackOverflowInterface with Serializable {
  ...
  def groupedPostings(postings: RDD[Posting]): RDD[(QID, Iterable[(Question, Answer)])] = { // TODO
    val questions: RDD[Posting] = postings.filter(_.postingType == 1)
    val qPrep: RDD[(QID, Question)] = questions.map(posting => (posting.id, posting))

    val answers: RDD[Posting] = postings.filter(_.postingType == 2)
    val aPrep: RDD[(QID, Answer)] = answers.map(posting => (posting.parentId.get, posting))

    qPrep.join(aPrep).groupByKey() // no shuffling up to here
  }
  ...
  def vectorPostings(scored: RDD[(Question, HighScore)]): RDD[(LangIndex, HighScore)] = {
    ...
    /* this changes all keys by the same constant factor, should not cause shuffling. */
  ...
  final def kmeans(means: Array[(Int, Int)],
    ...
    vectors
      ...
      .foreach(pair =>
        newMeans.update(pair._1, pair._2))  // filling in
  ...
  def clusterResults(means: Array[(Int, Int)], vectors: RDD[(LangIndex, HighScore)])
    ...
    val median = closestGrouped.mapValues { vs =>  // TODO

      val mostCommon = vs.groupBy(_._1)     // group by language index
                         .mapValues(_.size) // which language occurred how many times
                         .maxBy(_._2)       // most common language in the cluster

      val langLabel: String   = langs(mostCommon._1 / langSpread)
      val clusterSize: Int    = vs.size
      val langPercent: Double = (mostCommon._2.toDouble / clusterSize.toDouble) * 100.0

      val medianScore: Int    = {
        val vsScoresSorted = vs.map(_._2).toList.sorted
        val middle: Int = clusterSize / 2
        if (clusterSize % 2 == 0)
          (vsScoresSorted(middle - 1) + vsScoresSorted(middle)) / 2
        else vsScoresSorted(middle)
      }
