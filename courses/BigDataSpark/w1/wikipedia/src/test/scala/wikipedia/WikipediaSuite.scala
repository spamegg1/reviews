...
class WikipediaSuite extends munit.FunSuite:
  ...
  /** Creates a truncated string representation of a list, adding ", ...)" if there are
    * too many elements to show
    * @param l
    *   The list to preview
    * @param n
  ...
  def previewList[A](l: List[A], n: Int = 10): String =
    if l.length <= n then l.toString else l.take(n).toString.dropRight(1) + ", ...)"

  /** Asserts that all the elements in a given list and an expected list are the same,
    * regardless of order. For a prettier output, given and expected should be sorted with
    * the same ordering.
    * @param actual
    *   The actual list
    * @param expected
  ...
  def assertEquivalentAndOrdered(
    ...
    assertSameElements(actual, expected) // (1)
    assert( // (2)
