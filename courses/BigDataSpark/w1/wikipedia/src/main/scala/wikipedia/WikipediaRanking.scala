...
object WikipediaRanking extends WikipediaRankingInterface {
  ...
  val conf: SparkConf = new SparkConf() // TODO
    .setMaster("local[4]")
    .setAppName("MyApp")
  ...
  def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int = { // TODO
    ...
    rdd.aggregate(0)(fun, _+_) // aggregate is EAGER, not lazy!
  }
   ...
   *   several seconds.
   */
  def rankLangs(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] = { // TODO
    val langRankPairs =
      for (lang <- langs)
      yield (lang, occurrencesOfLang(lang, rdd))

    langRankPairs.sortBy(-_._2)
  }
   ...
   *   several seconds.
   */
  def rankLangsUsingIndex(index: RDD[(String, Iterable[WikipediaArticle])]) : List[(String, Int)] = // TODO
    index
      .mapValues(_.size)
      .collect
      .toList
   ...
   *   several seconds.
   */
  def rankLangsReduceByKey(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] = { // TODO
    def fun(article: WikipediaArticle) =
      for (lang <- langs.filter(article.mentionsLanguage))
      yield (lang, 1)
