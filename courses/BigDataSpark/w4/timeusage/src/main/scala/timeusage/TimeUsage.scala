...
object TimeUsage extends TimeUsageInterface {
  ...
  /** @return The initial data frame columns partitioned in three groups: primary needs (sleeping, eating, etc.),
    ...
    */
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = { // TODO
    val pList: List[String] = List("t01", "t03", "t11", "t1801", "t1803")
    val wList: List[String] = List("t05", "t1805")
    val oList: List[String] = List("t02", "t04", "t06", "t07", "t08", "t09",
                                   "t10", "t12", "t13", "t14", "t15", "t16", "t18")

    val columnsGrouped: Map[String, List[String]] =
      columnNames.groupBy(columnName =>
        if      (pList.exists(columnName.startsWith)) "primary"
        else if (wList.exists(columnName.startsWith)) "work"
        else if (oList.exists(columnName.startsWith)) "other"
        else                                          "none")
  ...
  /**
    ...
    */
  def timeUsageSummaryTyped(timeUsageSummaryDf: DataFrame): Dataset[TimeUsageRow] = // TODO
