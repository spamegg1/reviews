...
def history():
    ...
    # TODO
    # Query database for user's transactions in the transactions table
...
def login():
    ...
    if request.method == "POST":
        ...
        p = check_password_hash(rows[0]["hash"], request.form.get("password"))
        if len(rows) != 1 or not p:
