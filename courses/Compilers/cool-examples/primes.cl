...
class Main inherits IO {
    main(): Int { 0 };          -- main() is an atrophied method so we can parse

    -- out is our 'output'. Its values are the primes.
    out    : Int <- { out_string("2 is trivially prime.\n"); 2; };

    testee : Int <- out;                  -- a number to be tested for primeness
    divisor: Int;                            -- a number which may factor testee
    stop   : Int <- 500;                   -- an arbitrary value limiting testee
    ...
    m: Object <-                                  -- m supplants the main method
        while true loop
        {
            ...
            else        -- loop halted on testee/divisor = 0, testee isn't prime
