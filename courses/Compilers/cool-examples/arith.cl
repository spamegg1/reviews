...
class A {
    ...
    method1(num: Int): SELF_TYPE { self };                               -- same
...
(* The class A2I provides integer-to-string and string-to-integer conversion
   routines.  To use these routines, either inherit them in the class where
   needed, have a dummy variable bound to something of type A2I, or simply
   write (new A2I).method(argument). *)
class A2I {
    (*
        c2i
        Converts a 1-character string to an integer.
        Aborts if the string is not "0" through "9".
    *)
    c2i(char: String): Int {
...
(*  a2i
    converts an ASCII string into an integer. The empty string is converted to 0
    Signed and unsigned strings are handled. The method aborts if the string
    does not represent an integer. Very long strings of digits produce strange
    answers because of arithmetic overflow.
*)
    a2i(s: String): Int {
        if s.length() = 0 then 0 else
        if s.substr(0, 1) = "-" then
            ~a2i_aux(s.substr(1, s.length() - 1)) else
        if s.substr(0, 1) = "+" then
...
(* a2i_aux converts the unsigned portion of the string. As a programming example
...
class Main inherits IO {
    ...
    main() : Object {{
        ...
        while flag loop
        {
            ...
            if char = "a" then                                            -- add
            {
                a_var <- (new A).set_var(get_int());          -- declared at top
            ...
            else if char = "d" then                                 -- factorial
                avar <- (new C)@A.method5(avar.value())
            else if char = "e" then                                    -- square
                avar <- (new C)@B.method5(avar.value())
            else if char = "f" then                                      -- cube
