...
class Message(object):
    ...
    def build_shift_dict(self, shift):
        ...
        for i in range(26):
            ...
            shift_dict[LETTER] = string.ascii_uppercase[(i + shift) % 26]
    ...
    def apply_shift(self, shift):
        ...
        for letter in self.message_text:
            if letter.isalpha():
                cipher_text += self.build_shift_dict(shift)[letter]
            else:
                cipher_text += letter
...
class PlaintextMessage(Message):
    def __init__(self, text, shift):
        ...
        A PlaintextMessage object inherits from Message, has five attributes:
            self.message_text (string, determined by input text)
            self.valid_words (list, determined by helper function load_words)
...
class CiphertextMessage(Message):
    ...
    def decrypt_message(self):
        ...
        for shift_value in range(26):
            attempt = self.apply_shift(shift_value)

            # Count how many valid words there are in shifted message
            valid_word_count = 0
            for word in attempt.split(' '):
                if is_word(self.valid_words, word):
                    valid_word_count += 1

            # If current decryption is better, update the best
