...
@Points("01-07")
public class MessageThreeTimesTest {
    ...
    private void correctOutput(String input) {
        ...
        String[] lines = new String[]{"Write a message:", input, input, input};
        ...
        for (int i = 0; i < rows.size(); i++) {
            assertEquals("Line " + (i + 1) + " output was incorrect (when user input was " + input + ".\nExpecting output:\n" + lines[i] + "\nBut output was:\n" + rows.get(i), lines[i].trim(), rows.get(i).trim());
        }
    }
