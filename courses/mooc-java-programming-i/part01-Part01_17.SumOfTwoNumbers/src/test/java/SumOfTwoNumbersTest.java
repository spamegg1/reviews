...
public class SumOfTwoNumbersTest {
    ...
    private void correctOutput(String first, String second) {
        ...
        String[] lines = new String[]{"Give the first number:",
            ...
            "The sum of the numbers is " + (Integer.valueOf(first) + Integer.valueOf(second))
        };
        ...
        for (int i = 0; i < rows.size(); i++) {
            assertEquals("Line " + (i + 1) + " output was incorrect when input was:\n" + input + "\nExpecting output:\n" + lines[i] + "\nBut output was:\n" + rows.get(i), lines[i].trim(), rows.get(i).trim());
        }
    }

    @Test
    public void correctOrder() {
        List<String> code = code("SumOfTwoNumbers.java");
        int count = countOccurrences(code, "System.out.println.*Integer.*System.out.println.*Integer.*System.out.println");
        int count2 = countOccurrences(code, "System.out.println.*nextInt.*System.out.println.*Integer.*System.out.println");
