  ...
  val meldingHeaps: (String, Prop) =
    ...
    forAll { (heap1: List[Node], heap2: List[Node]) => // TODO
      ...
      def check(h1: List[Node], h2: List[Node], m: List[Node]): Boolean =
        ...
        else if h1.nonEmpty && findMin(m) == findMin(h1) then
          check(deleteMin(h1), h2, deleteMin(m))
        else if h2.nonEmpty && findMin(m) == findMin(h2) then
          check(h1, deleteMin(h2), deleteMin(m))
        else false
