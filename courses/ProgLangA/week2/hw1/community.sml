...
fun alternate_helper(numbers: int list, positive: bool): int =
    ...
    then (hd numbers) + alternate_helper (tl numbers, false)
    else alternate_helper(tl numbers, true) - (hd numbers)

fun alternate(numbers: int list): int = alternate_helper(numbers, true)
...
fun addAllOpt_helper(numbers: int option list): int =
    if null numbers
    then 0
    else if isSome (hd numbers)
    then addAllOpt_helper(tl numbers) + valOf(hd numbers)
    else addAllOpt_helper(tl numbers)

fun addAllOpt(numbers: int option list): int option =
let
    val total = addAllOpt_helper numbers
in
    if total = 0
    then NONE
    else SOME total
...
fun any(bools: bool list): bool =
    not(null bools) andalso (hd bools orelse any(tl bools))
...
fun all(bools: bool list): bool =
    null bools orelse (hd bools) andalso all(tl bools)
...
fun zipRecycle_helper(
    ...
    let
        val zipped   = if order then zip(nums1, nums2) else zip(nums2, nums1)
        val headless = head_remover(nums2, (length nums1))
        val rest     = zipRecycle_helper(nums1, headless, order)
    in
        zipped @ rest
    end
...
It then goes through the list of pairs
looking for the string s2 in the first component.
...
fun lookup(str_int_list: (string * int) list, s2: string): int option =
    if null str_int_list
    then NONE
    else if #1(hd str_int_list) = s2
    then SOME(#2 (hd str_int_list))
...
fun splitup(numbers: int list): int list * int list =
    ...
    let
        val list_pair: int list * int list = splitup(tl numbers)
        val pos = #1 list_pair
        val neg = #2 list_pair
    in
        if hd numbers < 0
        then (pos, hd numbers :: neg)
...
fun splitAt(numbers: int list, threshold: int): int list * int list =
    ...
    let
        val list_pair: int list * int list = splitAt ((tl numbers), threshold)
        val bigger  = #1 list_pair
        val smaller = #2 list_pair
    in
        if hd numbers < threshold
        then (bigger, hd numbers :: smaller)
...
that given a list of integers determines whether
the list is sorted in increasing order.
*)
fun isSorted(numbers: int list): bool =
    null numbers orelse
    null(tl numbers) orelse
    hd numbers < hd (tl numbers) andalso isSorted(tl numbers)
...
fun qsort(numbers: int list): int list =
    ...
    let
        val lists   = splitAt(tl numbers, hd numbers)
        val bigger  = #1 lists
        val smaller = #2 lists
    in
        qsort smaller @ hd numbers :: qsort bigger
        (*sortedMerge(qsort smaller, (hd numbers) :: qsort bigger)
        <- this also works*)
    end
...
fun divide(numbers: int list): int list * int list =
    ...
    let
        val lists: int list * int list = divide(tl (tl numbers))
        val firsts  = #1 lists
        val seconds = #2 lists
    in
...
fun fullDivide(pair: int * int): int * int =
    if (#2 pair) mod (#1 pair) <> 0
    then (0, #2 pair)
    else
    let
        val quotient = (#2 pair) div (#1 pair)
        val rest: int * int = fullDivide (#1 pair, quotient)
        val power   = #1 rest
        val divisor = #2 rest
    in
        (1 + power, divisor)
...
and the process should stop when the divisor considered
surpasses the square root of n.
If you make sure to use the reduced number n2 given
by fullDivide for each next step,
...
fun factorize_helper(current: int, divisor: int): (int * int) list =
    ...
    in
        ...
        if real_div > Math.sqrt real_cur
        then [(current, 1)]
        else
        let
        ...
        in
            ...
            then factorize_helper(quotient, divisor + 1)
...
For extra challenge, your recursive process should
return the numbers in this order,
