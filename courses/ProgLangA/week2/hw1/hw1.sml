...
fun number_in_month (datelist: (int * int * int) list, month: int): int =
    if null datelist then 0
    else let val x = number_in_month((tl datelist), month)
    in if month = (#2 (hd datelist)) then x + 1 else x end
...
fun number_in_months (datelist: (int * int * int) list, monthlist: int list): int =
    if null monthlist then 0 else
    number_in_months(datelist, (tl monthlist)) +
    number_in_month(datelist, (hd monthlist))
...
fun dates_in_month (datelist: (int * int * int) list, month: int)
    ...
    else let val x = dates_in_month((tl datelist), month)
    in if (#2 (hd datelist)) = month then (hd datelist) :: x else x end
...
fun oldest(datelist: (int * int * int) list): (int * int * int) option =
