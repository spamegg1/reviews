...
(see http://sml-family.org/Basis/list.html#SIG:LIST.foldr:VAL).
...
let
    val countdown = fn n => if n = 0 then NONE else SOME (n, n - 1)
    val mult = fn (x, y) => x * y
in
    foldr mult 1 (unfold countdown n)
end
...
let
    fun binOp (a, alist) = if (pred a) then (a :: alist) else alist
in
    foldr binOp [] lst
end
