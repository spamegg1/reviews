...
val test7a = first_answer (fn x => if x > 0 then SOME x else NONE) [1,2,3,4,5] = 1
val test7b = first_answer (fn x => if x > 1 then SOME x else NONE) [1,2,3,4,5] = 2
val test7c = first_answer (fn x => if x > 2 then SOME x else NONE) [1,2,3,4,5] = 3
val test7d = first_answer (fn x => if x > 3 then SOME x else NONE) [1,2,3,4,5] = 4
val test7e = first_answer (fn x => if x > 4 then SOME x else NONE) [1,2,3,4,5] = 5
val test7f = ((first_answer (fn x => if x > 5 then SOME x else NONE) [1,2,3,4,5]; false)
...
val pat8 = TupleP [pat1, TupleP [pat5, pat2], pat2, pat7, TupleP [pat7, pat3, pat4], pat6]
...
val test11j = match (Tuple[Const 17,Unit,Const 4,Constructor ("egg",Const 4),Constructor ("egg",Constructor ("egg",Const 4))],TupleP[Wildcard,Wildcard]) = NONE
