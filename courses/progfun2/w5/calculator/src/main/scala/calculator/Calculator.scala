...
object Calculator extends CalculatorInterface:
  ...
  def eval(expr: Expr, references: Map[String, Signal[Expr]])(using
    ...
    expr match
      ...
      case Divide(a, b) =>
        eval(b, references) match
          case 0     => Double.NaN
          case denom => eval(a, references) / denom

  /** Get the Expr for a referenced variables. If the variable is not known, returns a
    * literal NaN.
    */
  private def getReferenceExpr(name: String, references: Map[String, Signal[Expr]])(using
      Signal.Caller
  ): Expr =
    references
      .get(name)
