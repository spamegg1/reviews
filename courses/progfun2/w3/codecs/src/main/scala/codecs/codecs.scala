...
/** A data type modeling JSON values. For example, the `42` integer JSON value can be
  * modeled as `Json.Num(42)`
  */
enum Json:
  case Null /* The JSON `null` value */
  case Bool(value: Boolean) /* JSON boolean values */
  case Num(value: BigDecimal) /* JSON numeric values */
  case Str(value: String) /* JSON string values */
  case Obj(fields: Map[String, Json]) /* JSON objects */
  case Arr(items: List[Json]) /* JSON arrays */

  /** Try to decode this JSON value into a value of type `A` by using the contextual
    * decoder. Note that you have to explicitly fix `A` type parameter when you call the
    * method:
    *
    * {{{
...
trait Encoder[-A]:
  ...
  /** Transforms this `Encoder[A]` into an `Encoder[B]`, given a transformation function
    * from `B` to `A`. For instance, given a `Encoder[String]`, we can get an
    * `Encoder[UUID]`:
    *
    * {{{
...
object Encoder extends EncoderInstances:
  /** Convenient method for creating an instance of encoder from a function `f`
    */
  infix def fromFunction[A](f: A => Json) = new Encoder[A]:
...
trait EncoderInstances:
  ...
  given Encoder[String] = Encoder fromFunction (string => Json.Str(string))

  /** An encoder for `Boolean` values */
  // TODO Define a given value of type `Encoder[Boolean]`
  given Encoder[Boolean] = Encoder fromFunction (boolean => Json.Bool(boolean))

  /** Encodes a list of values of type `A` into a JSON array containing the list elements
    * encoded with the given `encoder`
    */
...
trait ObjectEncoder[-A] extends Encoder[A]:
  ...
  /** Combines `this` encoder with `that` encoder. Returns an encoder producing a JSON
    * object containing both fields of `this` encoder and fields of `that` encoder.
    */
...
object ObjectEncoder:
  /** Convenient method for creating an instance of object encoder from a function `f`
    */
  infix def fromFunction[A](f: A => Json.Obj): ObjectEncoder[A] =
    new ObjectEncoder[A]:
      def encode(value: A): Json.Obj = f(value)

  /** An encoder for values of type `A` that produces a JSON object with one field named
    * according to the supplied `name` and containing the encoded value.
    */
...
trait Decoder[+A]:
  ...
  /** Combines `this` decoder with `that` decoder. Returns a decoder that invokes both
    * `this` decoder and `that` decoder and returns a pair of decoded value in case both
    * succeed, or `None` if at least one failed.
    */
...
object Decoder extends DecoderInstances:
  ...
  /** Convenient method to build a decoder instance from a function `f`
    */
  infix def fromFunction[A](f: Json => Option[A]): Decoder[A] = new Decoder[A]:
...
trait DecoderInstances:
  ...
  given intDecoder: Decoder[Int] = // TODO
    Decoder.fromFunction: json =>
      json match
        case Json.Num(value) => if value.isValidInt then Some(value.toInt) else None
        case _               => None
  ...
  /** A decoder for JSON arrays. It decodes each item of the array using the given
    * `decoder`. The resulting decoder succeeds only if all the JSON array items are
  ...
  /** A decoder for JSON objects. It decodes the value of a field of the supplied `name`
