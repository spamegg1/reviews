...
object Util:
  ...
  /** Parse the JSON value from the supplied `s` parameter, and then try to decode it as a
    * value of type `A` using the given `decoder`. Returns `None` if JSON parsing failed,
  ...
  private def render(json: Json): String = json match
    ...
    case Json.Obj(vs) =>
      vs
        .map((k, v) => s"${renderString(k)}:${render(v)}")
        .mkString("{", ",", "}")
  ...
  private def renderString(s: String): String =
    ...
    while i < len do
      s.charAt(i) match
        ...
        case '\t' => sb.append("\\t")
        case c =>
          if c < ' ' then sb.append("\\u%04x" format c.toInt) else sb.append(c)
      i += 1
    sb.append('"').toString
