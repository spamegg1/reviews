...
trait GameDef:
  ...
  /** The case class `Pos` encodes positions in the terrain.
    ...
    *
    * Illustration: # is at position Pos(2, 1)
    *
    * * 0 1 2 3 <- col axis
    *
    * 0 o o o o
  ...
  /** The position where the block is located initially. This value is left abstract, it
    * will be defined in concrete instances of the game.
    */
  def startPos: Pos

  /** The target position where the block has to go. This value is left abstract.
    */
  def goal: Pos

  /** The terrain is represented as a function from positions to booleans. The function
    * returns `true` for every position that is inside the terrain. As explained in the
    * documentation of class `Pos`, the `row` axis is the vertical one and increases from
    * top to bottom.
    */
  ...
  /** In Bloxorz, we can move left, right, Up or down. These moves are encoded as case
    * objects.
    */
  enum Move:
  ...
  /** A block is represented by the position of the two cubes that it consists of. We make
    * sure that `b1` is lexicographically smaller than `b2`.
    */
  case class Block(b1: Pos, b2: Pos):
    require( // checks the requirement mentioned above
      b1.row <= b2.row && b1.col <= b2.col,
      "Invalid block position: b1=" + b1 + ", b2=" + b2
    )

    /** Returns a block where the `row` coordinates of `b1` and `b2` are changed by `d1`
      * and `d2`, respectively.
      */
    def deltaRow(d1: Int, d2: Int) = Block(b1.deltaRow(d1), b2.deltaRow(d2))

    /** Returns a block where the `col` coordinates of `b1` and `b2` are changed by `d1`
      * and `d2`, respectively.
      */
    ...
    /** Returns the list of blocks that can be obtained by moving the current block,
      * together with the corresponding move.
      */
    def neighbors: List[(Block, Move)] = // TODO
      List((left, Move.Left), (right, Move.Right), (up, Move.Up), (down, Move.Down))

    /** Returns the list of positions reachable from the current block which are inside
      * the terrain.
      */
    def legalNeighbors: List[(Block, Move)] = neighbors.filter(_._1.isLegal) // TODO
