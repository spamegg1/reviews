...
class KMeansSuite extends munit.FunSuite:
  ...
  test(
    "'classify' with data parallelism should work for empty 'points' and 'means' == Seq(Point(1,1,1))"
  ):
    val points: ParSeq[Point] = IndexedSeq().par
  ...
  test(
    "'classify' with data parallelism should work for 'points' == Seq((1, 1, 0), (1, -1, 0), (-1, 1, 0), (-1, -1, 0)) and 'means' == Seq((0, 0, 0))"
  ):
  ...
  test(
    "'classify' with data parallelism should work for 'points' == ParSeq((1, 1, 0), (1, -1, 0), (-1, 1, 0), (-1, -1, 0)) and 'means' == Seq((1, 0, 0), (-1, 0, 0))`"
  ):
    val p1: Point = new Point(1, 1, 0)
    val p2: Point = new Point(1, -1, 0)
    val p3: Point = new Point(-1, 1, 0)
  ...
  def checkParKMeans(
      ...
      means: ParSeq[Point],
      eta: Double,
      expected: ParSeq[Point]
  ): Unit =
    assertEquals(
      kMeans(points, means, eta).toString,
      expected.toString,
      s"kMeans($points, $means, $eta) should equal to $expected"
    )

  test(
    "'kMeans' with data parallelism should work for 'points' == Seq((0, 0, 1), (0,0, -1), (0,1,0), (0,10,0)) and 'oldMeans' == Seq((0, -1, 0), (0, 2, 0)) and 'eta' == 12,25"
  ):
