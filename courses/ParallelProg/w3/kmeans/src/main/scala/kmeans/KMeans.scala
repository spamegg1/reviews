...
class KMeans extends KMeansInterface:
  ...
  def classify(
      points: ParSeq[Point],
      means: ParSeq[Point]
  ): ParMap[Point, ParSeq[Point]] = // TODO
    val ptsGp: ParMap[Point, ParSeq[Point]] = points.groupBy(findClosest(_, means))
  ...
  def update(
    ...
    val res = oldMeans.map(oldMean => findAverage(oldMean, classified(oldMean)))
  ...
  final def kMeans(
    ...
    if !converged(eta, means, newMeans) then kMeans(points, newMeans, eta)
    else newMeans

/** Describes one point in three-dimensional space. Note: deliberately uses reference
  * equality.
  */
...
object KMeansRunner:
  ...
  def main(args: Array[String]): Unit =
    ...
    val seqtime =
      ...
      try // Run the the algorithm on the sequential support
      ...
      finally // Restore the parallel support
        points.tasksupport = parTasksupport
        means.tasksupport = parTasksupport
        seqPool.shutdown() // Stop the sequential runner
