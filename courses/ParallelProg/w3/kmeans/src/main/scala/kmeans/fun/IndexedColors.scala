...
class IndexedColorFilter(
  ...
  private val newMeans = kMeans(points, means, 0.01) /* The work is done here: */

  def getStatus() = s"Converged after $steps steps." /* these are the results exposed */
    ...
    val initialPoints: Seq[Point] =
      initStrategy match
        ...
        case UniformSampling =>
          val sep: Int = 32
          (for
            r <- 0 until 255 by sep;
            g <- 0 until 255 by sep;
  ...
  private def computeSNR(points: ParSeq[Point], means: ParSeq[Point]): Double =
    ...
    for point <- points do
      ...
      noise += sqrt(
        pow(point.x - closest.x, 2) + pow(point.y - closest.y, 2)
          + pow(point.z - closest.z, 2)
      )
    sound / noise
  ...
  override def converged(
    ...
    steps += 1
    convStrategy match
      case ConvergedAfterNSteps(n)          => steps >= n
      case ConvergedAfterMeansAreStill(eta) => super.converged(eta, oldMeans, newMeans)
