...
object VerticalBoxBlur extends VerticalBoxBlurInterface:
  /** Blurs the columns of the source image `src` into the destination image `dst`,
    * starting with `from` and ending with `end` (non-inclusive). Within each column,
    * `blur` traverses the pixels by going from top to bottom.
    */
  ...
  /** Blurs the columns of the source image in parallel using `numTasks` tasks.
    * Parallelization is done by stripping the source image `src` into `numTasks` separate
    * strips, where each strip is composed of some number of columns.
    */
  def parBlur(src: Img, dst: Img, numTasks: Int, radius: Int): Unit = // TODO
    ...
    yield task { blur(src, dst, from, endPt, radius) }
