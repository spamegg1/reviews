...
case class Fork(nw: Quad, ne: Quad, sw: Quad, se: Quad) extends Quad:
  ...
  val weightX = // MINE
    nw.mass * nw.massX + ne.mass * ne.massX + sw.mass * sw.massX + se.mass * se.massX
  val weightY = // MINE
    nw.mass * nw.massY + ne.mass * ne.massY + sw.mass * sw.massY + se.mass * se.massY
...
case class Leaf(centerX: Float, centerY: Float, size: Float, bodies: coll.Seq[Body])
  ...
  val (mass, massX, massY) = (massCalc, weightX / massCalc, weightY / massCalc) // TODO
...
class Body(
  ...
  def updated(quad: Quad): Body =
    ...
    def traverse(quad: Quad): Unit = quad match
      ...
      case Leaf(_, _, _, bodies) => // TODO
        // add force contribution of each body by calling addForce
        for body <- bodies do addForce(body.mass, body.x, body.y)
      case Fork(nw, ne, sw, se) => // TODO
        // see if node is far enough from the body, or if recursion is needed
        if quad.size / distance(x, y, quad.massX, quad.massY) < theta then
          addForce(quad.mass, quad.massX, quad.massY)
        else traverse(nw); traverse(ne); traverse(sw); traverse(se) // recur on subquads
...
class SectorMatrix(val boundaries: Boundaries, val sectorPrecision: Int)
  ...
  def +=(b: Body): SectorMatrix = // TODO
    val secX =
      if b.x <= boundaries.minX then boundaries.minX
      else if b.x >= boundaries.maxX then boundaries.maxX - 1
      else b.x
    val secY =
      if b.y <= boundaries.minY then boundaries.minY
      else if b.y >= boundaries.maxY then boundaries.maxY - 1
      else b.y

    val sectorX =
      (secX - boundaries.minX) / (boundaries.maxX - boundaries.minX) * sectorPrecision

    val sectorY =
      (secY - boundaries.minY) / (boundaries.maxY - boundaries.minY) * sectorPrecision

    matrix(sectorY.toInt * sectorPrecision + sectorX.toInt) += b // using ConcBuffer's +=
    this
  end +=
...
class TimeStatistics:
  ...
  override def toString = timeMap
    .map { case (k, (total, num)) => s"$k: ${(total / num * 100).toInt / 100.0} ms" }
    .mkString("\n")
