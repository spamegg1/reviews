  ...
  def init2Galaxies(totalBodies: Int): Unit =
    ...
    def galaxy(
      ...
      for i <- from until (from + num) do
        val b =
          if i == from then Body(blackHoleM, cx, cy, sx, sy)
          else
            ...
            val speed = math.sqrt(
              gee * blackHoleM / radius + gee * totalM * radius * radius / cubmaxradius
            )
    ...
    screen = Boundaries() // compute center and boundaries
    screen.minX = -2200.0f
    screen.minY = -1600.0f
    screen.maxX = 350.0f
    screen.maxY = 350.0f
