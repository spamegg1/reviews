...
object FloatOps:
  ...
  extension (self: Float)
    def ~=(that: Float): Boolean = abs(self - that) < precisionThreshold

  /** Long floating comparison: assert(double ~= 1.7). */
  extension (self: Double)
    def ~=(that: Double): Boolean = abs(self - that) < precisionThreshold

  /** Floating sequences comparison: assert(floatSeq ~= Seq(0.5f, 1.7f). */
  extension (self: Seq[Float])
    def ~=(that: Seq[Float]): Boolean = self.size == that.size &&
      self.zip(that).forall((a, b) => abs(a - b) < precisionThreshold)
