...
class ParenthesesBalancingSuite extends munit.FunSuite:
  ...
  def check(input: String, expected: Boolean) = // sequential
  ...
  test("balance: '(if (zero? x) max (/ 1 x))' is balanced"): // sequential
  ...
  test("parBalance: '(if (zero? x) max (/ 1 x))' is balanced"): // parallel
