...
found:
  p->state = EMBRYO;
  p->pid = nextpid++;
  p->tickets = 1;		// OSTEP project: by default get 1 ticket
...
fork(void)
{
  ...
  np->parent = curproc;
  *np->tf = *curproc->tf;
  np->tickets = curproc->tickets;		// OSTEP project: make child inherit tickets
...
// OSTEP project: random seed for generator below
unsigned long int seed = 1;

// OSTEP project: update random seed value
void
srand(uint n)
{
  seed = n;
...
// OSTEP project: generate a pseudorandom number from a random seed
int
rand(void)
{
...
// OSTEP project: return a pseudorandom number between 0 and n
int
getwinner(int n)
{
	return (rand() % (n+1));
...
// OSTEP project: set the number of tickets for the calling process,
// to be used by the lottery scheduler
int
